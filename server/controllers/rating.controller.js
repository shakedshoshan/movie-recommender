...
exports.setRatings = async (req, res) => {
  try {
    ...
    const responseFromDb = await ratingService.setRating({
      userId: userId,
...
exports.getRatings = async (req, res) => {
  try {
    const userId = await authService.getUserIdFromToken(req.body.token);
    const responseFromDb = await ratingService.getRating({
      userId: userId,
      movieId: req.body.movieId,
    });
    res.status(200).json({ rating: { responseFromDb } });
...
exports.getAllRatings = async (req, res) => {
  try {
    ...
    const responseFromDb = await ratingService.getAllRating({
