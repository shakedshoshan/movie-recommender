    ...
    try {
        ...
        if(ratings){
            ...
            if(dataForAlgorithem){
                const responseFromAlgorithem = await recommendationService.generateRecommendation({data: dataForAlgorithem});
                if(responseFromAlgorithem){
                    const responseFromDb = await recommendationService.saveRecommendationToDb({userId: userId, recommendation: responseFromAlgorithem.prediction});
                    if(responseFromDb){
                        res.status(200).json({ recommendation: responseFromDb.recommendations });
                    }
                    else {
                ...
                res.status(400).json({ error: "Failed to prepare data for algorithem" });
            }
