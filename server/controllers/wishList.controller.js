...
exports.addToWishList = async (req, res) => {
  try {
    ...
    const responseFromDb = await wishListService.addToWishList({
      userId: userId,
...
exports.removeFromWishList = async (req, res) => {
  try {
    ...
    const responseFromDb = await wishListService.removeFromWishList({
      userId: userId,
      movieId: req.body.movieId,
    });
...
exports.getWishList = async (req, res) => {
  try {
    ...
    const responseFromDb = await wishListService.getWishList({
      userId: userId,
    });
    res.status(200).json({ wishList: { responseFromDb } });
  } catch (error) {
...
exports.checkIfExistsIntWishList = async (req, res) => {
  try {
    const userId = await authService.getUserIdFromToken(req.body.token);
    const responseFromDb = await wishListService.checkIfExistsIntWishList({
      userId: userId,
      movieId: req.body.movieId,
    });
    res.status(200).json({ wishList: responseFromDb });
